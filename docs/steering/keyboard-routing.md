...
- グローバル状態（例: `ui.store.textInputFocused: boolean`）でフォーカス状態を共有
- TextInput を表示する Screen が mount/unmount 時に flag を立て下げ
...
1. `ui.store.textInputFocused === true` ではないか？ → Layer 3 で吸われている
