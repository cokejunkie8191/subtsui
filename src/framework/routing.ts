// src/framework/routing.ts
import type { Screen, KeyEvent } from './Screen'

export type RouteDecision =
  | 'blocked'      // Layer 3 が遮断（Esc/Tab 以外）
...
export function decideRoute(
  ...
  // Layer 3: TextInput active。Esc と Tab はナビゲーション用に通す
  if (ctx.textInputFocused && !e.key.escape && !e.key.tab) return 'blocked'
